  ...
  return (
    <nav className="bottom-nav" role="navigation" aria-label="Primary">
      {items.map(({ id, label, Icon }) => {
        // A cart the user cannot see is a cart they forget. The count rides the
        // tab itself so it is visible from every screen, not just the shop.
        const badge = id === 'shubh-kart' && count > 0 ? count : null;
        return (
          <button
            key={id}
            className={active === id ? 'active' : ''}
            onClick={() => onNavigate(id === 'menu' ? 'settings' : id)}
            aria-label={badge ? `${label} (${badge} item${badge > 1 ? 's' : ''} in cart)` : undefined}
          >
            <span className="nav-icon">
              <Icon size={22} />
              {badge && <em className="nav-count">{badge > 9 ? '9+' : badge}</em>}
            </span>
            <span>{label}</span>
          </button>
        );
